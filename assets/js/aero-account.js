...
(function () {
    ...
    async function getProfile(userId) {
        const { data, error } = await sb.from("profiles").select("display_name,username,avatar,gender,unlocked_achievements,achievement_level").eq("id", userId).single();
        if (error) { console.error(error); return null; }
        return data;
    }

    // Same tier logic as Bubbles' js/app.js — kept in sync manually since
    // Aero World doesn't load Bubbles' code, just reads the same columns.
    const STATUS_TIERS = [
    ...
    function getStatusTier(profile) {
        const count = profile?.achievement_level || 0;
        let tier = null;
        for (const t of STATUS_TIERS) if (count >= t.min) tier = t;
        if (!tier) return null;
        return { icon: tier.icon, title: typeof tier.title === "function" ? tier.title(profile.gender) : tier.title };
    }

    function showAeroToast(text) {
        const t = el("div", { class: "aero-account-toast" }, text);
        document.body.appendChild(t);
        requestAnimationFrame(() => t.classList.add("show"));
        setTimeout(() => { t.classList.remove("show"); setTimeout(() => t.remove(), 400); }, 4000);
    }

    // Grants an achievement id straight to the shared `profiles` row —
    // safe to call repeatedly, it no-ops once the id is already unlocked.
    // Exposed on window so any page (mini-games, guestbook, etc.) can call
    // window.grantAeroAchievement(...) once aero-account.js has loaded.
    async function grantAeroAchievement(id, { icon, title } = {}) {
        const { data: { session } } = await sb.auth.getSession();
        if (!session) return;
        const { data: profile, error } = await sb.from("profiles").select("unlocked_achievements,achievement_level").eq("id", session.user.id).single();
        if (error || !profile) { console.error(error); return; }
        const current = Array.isArray(profile.unlocked_achievements) ? profile.unlocked_achievements : [];
        if (current.includes(id)) return;
        const updated = [...current, id];
        const { error: updateError } = await sb.from("profiles")
            .update({ unlocked_achievements: updated, achievement_level: updated.length })
            .eq("id", session.user.id);
        if (updateError) { console.error(updateError); return; }
        showAeroToast(`🏆 Новое достижение: ${icon || "🏆"} ${title || id}!`);
        renderWidget();
    }
    window.grantAeroAchievement = grantAeroAchievement;
    ...
    function buildLoggedInBadge(host, profile, userId) {
        ...
        const tier = getStatusTier(profile);
        ...
        return el("div", { class: "aero-account-badge" },
            avatar,
            tier ? el("span", { class: "aero-account-tier", title: tier.title }, tier.icon) : null,
    ...
    async function renderWidget(host) {
        ...
        grantAeroAchievement("aero_visitor", { icon: "🌐", title: "Гость Aero World" });
    }
